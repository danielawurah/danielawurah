...
This project implements a **Zero-Touch Joiner-Mover-Leaver (JML) automation** pipeline — mapping HR-sourced user attributes to dynamic group memberships in Okta, which drive automatic **Birthright Access provisioning and revocation** across connected applications.

When a new employee joins, their AD attributes (department, job title, location) automatically place them into the correct Okta groups — and those groups grant app access instantly, with no manual ticket. When an employee changes role (Mover) or leaves (Leaver), attribute changes trigger real-time group recalculation and access revocation. No standing access lingers.

This is the **identity lifecycle backbone** that underpins every other IAM project in this portfolio — the system that determines who has access, to what, and for how long.
...
| `05-birthright-app-assignment.png` | Application auto-assigned to user via group membership — no manual action |
...
| `08-leaver-account-disabled.png` | AD account disabled — Okta session terminated, all app access revoked |
| `09-scim-provisioning-log.png` | SCIM provisioning log showing downstream app receiving the lifecycle event |
| `10-audit-trail.png` | Okta System Log — full audit trail of provisioning and deprovisioning events |
...
| **Sync Mechanism** | Okta AD Agent — attribute sync from AD to Okta |
| **Access Control Model** | ABAC — Attribute-Based Access Control via Okta Group Rules |
| **Provisioning Protocol** | SCIM 2.0 — downstream app provisioning |
| **Automation** | PowerShell — AD attribute manipulation and lifecycle simulation |
...
7. SCIM pushes account creation to downstream apps (e.g., Salesforce, Slack, internal tools)
         ↓
8. User logs in on Day 1 — everything is already there
...
3. Okta re-evaluates group rules — user removed from "Analyst-Role", added to "Senior-Analyst-Role"
         ↓
4. App assignments recalculate — new apps granted, old apps revoked automatically
         ↓
...
| **ABAC** | Attribute-Based Access Control — access decisions driven by user attributes, not manual role assignment |
| **Birthright Access** | The baseline set of apps every employee in a given role automatically receives on Day 1 |
| **Group Rule** | Okta logic engine — evaluates user attributes and auto-assigns users to groups using expressions |
| **SCIM** | System for Cross-domain Identity Management — protocol for automated provisioning to downstream apps |
...
| **Least Privilege** | Users hold only the access their current attributes entitle them to — no excess, no accumulation |
...
- Used **PowerShell** to simulate lifecycle events:
  ...
  - Mover simulation — attribute update (department/title change)
  - Leaver simulation — account disable
...
### SCIM Provisioning — Downstream App Lifecycle
