...
This project replaces the traditional VPN model with a **Cloudflare Tunnel-based Zero Trust Network Access (ZTNA)** architecture — enforcing identity-verified, least-privilege access to internal Active Directory resources **without exposing any service to the public internet**.

Instead of granting broad network-level access after a single VPN login, every request must be authenticated against a verified identity (AD-backed via Okta), pass device posture checks, and match a defined access policy before reaching the internal resource. The network perimeter is effectively eliminated — trust is never assumed, always earned at the application layer.
...
> No inbound firewall rules. No VPN client. No public IP. The internal server calls out — Cloudflare routes in.
...
1. User navigates to internal app URL (e.g., internal.corp.example.com)
         ↓
2. Cloudflare Access intercepts — redirects to Okta login
         ↓
...
7. Policy PASS → Request proxied through Cloudflare Tunnel to internal resource
   Policy FAIL → Request blocked — 403 returned, no tunnel traffic initiated
         ↓
8. Session logged — user identity, device, timestamp, policy outcome all recorded
```
...
| **NIST SP 800-207** | NIST's Zero Trust Architecture standard — defines principles this deployment follows |
...
- Policy denies access silently if any condition is not met — no indication of internal resource existence

### Validation & Testing
- Verified **authorized user access** — authenticated through Okta + MFA → landed on internal resource
- Verified **unauthorized user block** — user outside the policy group received 403 — no resource exposed
...
- Reviewed **session logs** in Cloudflare Access — confirmed user identity, device, and policy decision logged for each access event
...
- **VPN Elimination** — Internal resources accessible with zero inbound firewall rules, zero public IP exposure
...
- **Audit Logging** — Every session is logged with identity, device, timestamp, and policy outcome — full accountability trail
